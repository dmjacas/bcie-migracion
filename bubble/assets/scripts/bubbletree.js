...
var vis4color = function (mode) {
    ...
    this.lightness = function (l) {
        if (this.mode != "hsl") {
            this.log("WARNING: lightness property not available in " + this.mode + " mode");
            return;
        }
        this._evaluate(l, "l");
        this.hsx2rgb();
    ...
    this.brightness = function (br) {
        if (this.mode != "hsb") {
            this.log("WARNING: brightness property not available in " + this.mode + " mode");
            return;
        }
    ...
    this.value = function (v) {
        if (this.mode != "hsv") {
            this.log("WARNING: value property not available in " + this.mode + " mode");
            return;
        }
        this._evaluate(v, "v");
        this.hsx2rgb();
    ...
    this.intensity = function (i) {
        if (this.mode != "hsi") {
            this.log("WARNING: intensity property not available in " + this.mode + " mode");
            return;
        }
        this._evaluate(i, "i");
        this.hsx2rgb();
        ...
        if (node !== null) {
            ...
            if (node == root || node.parent == root && node.children.length < 2) {
                ...
                rad2 = 0 - Math.max(
                    ...
                    hw * 0.8 - tgtScale * (a2rad(node.parent.amount) + a2rad(Math.max(node.amount * 1.15 + node.maxChildAmount * 1.15, node.left.amount * 0.85, node.right != undefined ? node.right.amount * 0.85 : 0))),
...
BubbleTree.Utils.formatNumber = function (n) {
    ...
    if (n >= 1000000000000) return prefix + Math.round(n / 100000000000) / 10 + 'trillones';
    if (n >= 1000000000) return prefix + Math.round(n / 100000000) / 10 + 'billones';
    if (n >= 1000000) return prefix + Math.round(n / 100000) / 10 + ' millones';
    if (n >= 1000) return prefix + Math.round(n / 100) / 10 + ' miles';
...
BubbleTree.Bubbles.Donut = function (node, bubblechart, origin, radius, angle, color) {
    ...
    this.draw = function () {
        var me = this, r = Math.max(5, me.bubbleRad * me.bc.bubbleScale), ox = me.pos.x, oy = me.pos.y,
            devnull = me.getXY(), showLabel = r > 20, x = me.pos.x, y = me.pos.y;
        if (!me.visible) return;
...
BubbleTree.Bubbles.Icon = function (node, bubblechart, origin, radius, angle, color) {
    ...
    me.show = function () {
        ...
        if (me.node.id == 'root') {
            me.dashedBorder = me.paper.circle(cx, cy, Math.min(r - 3, r * 0.95))
                .attr({stroke: '#ac2925', 'stroke-dasharray': "*"});
        }
        else {
            me.dashedBorder = me.paper.circle(cx, cy, Math.min(r - 3, r * 0.95))
                .attr({stroke: '#ffffff', 'stroke-dasharray': "- "});
        ...
        // additional label
        me.label2 = $('<div class="bubbletree-label2 ' + me.node.id + '"><span>' + me.node.shortLabel + '</span></div>');
        me.label3_1 = $('<div class="bubbletree-label2 ' + me.node.id + '"><span>' + me.node.shortLabel + ': ' + utils.formatNumber(me.node.amount) + '</span></div>');
        me.bc.$container.append(me.label2);
    ...
    me.draw = function () {
        ...
        if (!showLabel) {
            me.label.hide();
            // console.log('3-1');
            console.log(me.label3_1);
            // me.label2 = me.label3_1;
            me.label2.show();
        } else {
            me.label.show();
